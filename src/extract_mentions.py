...
def generate_mentions(client, segments_csv, output_txt, log_folder, call_count, start_time):
    """Processes the segments CSV file and generates mentions.
    Args:
    ...
    with open(segments_csv, "r", encoding="utf-8") as segments_file:
        ...
        segments = [(row['inicio'], row['texto']) for row in reader]
        
    call_count = 0
    start_time = time.time()
    for segment in tqdm(segments):
        response, call_count, start_time = rate_limited_api_call(client, segment[1], call_count, start_time, log_folder)
        cleaned_text = response.text.replace("```xml", "").replace("```", "")
        if cleaned_text.strip() != "<NADA/>":
            with open(output_txt, "a", encoding="utf-8") as output_file:            
                output_file.write(f"{segment[0]}:{cleaned_text}\n")
    return call_count, start_time

if __name__ == "__main__":
    with open("prompts/system_prompt_mentions.txt", "r", encoding="utf-8") as f:
    ...
    OUTPUT_TXTs = [
        "annotations/mentions/1993-05-24.txt",
        "annotations/mentions/2008-02-25.txt",
        "annotations/mentions/2008-03-03.txt",
        "annotations/mentions/2011-11-07.txt",
        "annotations/mentions/2015-11-23.txt",
        "annotations/mentions/2015-11-30.txt",
        "annotations/mentions/2015-12-14.txt",
        "annotations/mentions/2016-06-13.txt",
        "annotations/mentions/2019-04-16.txt",
        "annotations/mentions/2019-04-20.txt",
        "annotations/mentions/2019-04-22.txt",
        "annotations/mentions/2019-04-23.txt",
        "annotations/mentions/2019-11-01.txt",
        "annotations/mentions/2019-11-02.txt",
        "annotations/mentions/2019-11-04.txt",
        "annotations/mentions/2019-11-07.txt",
        "annotations/mentions/2023-07-10.txt",
        "annotations/mentions/2023-07-13.txt",
        "annotations/mentions/2023-07-19.txt"        
    ]
    ...
    for SEGMENTS_CSV, OUTPUT_TXT in zip(SEGMENTS_CSVs, OUTPUT_TXTs):
        # Si SEGMENTS_CSV no existe, abortamos la ejecución
        if not os.path.exists(SEGMENTS_CSV):
            print("File not found:", SEGMENTS_CSV)
            exit(1)

        # Si OUTPUT_TXT ya existe, no lo volvemos a procesar
        if os.path.exists(OUTPUT_TXT):
            print("Skipping", SEGMENTS_CSV)
            continue

        print("Processing", SEGMENTS_CSV)
        call_count, start_time = generate_mentions(client, SEGMENTS_CSV, OUTPUT_TXT, LOG_FOLDER, call_count, start_time)
